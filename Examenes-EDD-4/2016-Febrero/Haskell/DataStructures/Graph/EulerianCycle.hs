...
extractCycle g v0 = extractCycleRec g v0 [v0]
    where 
        extractCycleRec g v ciclo
            | v == v0 && length ciclo > 1 = (g, ciclo)
            | otherwise                   = extractCycleRec (remove g (v,u)) u (ciclo ++ [u]) 
                where 
...
eulerianCycle g 
    ...
    | otherwise         = eulerianCycleRec g [] (head (vertices g))
        where 
            eulerianCycleRec g xs v
                | isEmpty g = xs 
                | otherwise = eulerianCycleRec g' (connectCycles xs ys) u
                    where 
                        (g', ys) = extractCycle g v 
                        u        = vertexInCommon g' ys 
