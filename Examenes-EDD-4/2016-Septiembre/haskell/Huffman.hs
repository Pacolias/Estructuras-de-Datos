...
weights palabra = foldr (\c -> D.insert c (length [x | x <- palabra, x == c])) D.empty palabra
...
huffmanLeaves palabra = foldr (\(c, n) -> PQ.enqueue (WLeaf c n)) PQ.empty (D.keysValues (weights palabra))
...
huffmanTree palabra
  | D.size (weights palabra) < 2 = error "the string must have at least two different symbols"
  | otherwise                    = huffmanTreeRec (huffmanLeaves palabra)
    where 
      huffmanTreeRec pq 
        | PQ.isEmpty (PQ.dequeue pq) = PQ.first pq 
        | otherwise                  = huffmanTreeRec colaParametro
          where 
            primero = PQ.first pq 
            segundo = PQ.first (PQ.dequeue pq)
            mezcla = merge primero segundo 
            colaParametro = PQ.enqueue mezcla (PQ.dequeue (PQ.dequeue pq))
...
joinDics dict1 dict2 = foldr (uncurry D.insert) dict2 (D.keysValues dict1)
...
prefixWith x dict = foldr (\(k, xs) -> D.insert k (x:xs)) D.empty (D.keysValues dict)
...
takeSymbol [] (WLeaf c _)         = (c, [])
takeSymbol [] (WNode lt rt _)     = (' ', [])
takeSymbol codigo (WLeaf c _)     = (c, codigo)
...
decode [] _               = []
decode codigo huffmanTree = c : decode restoCodigo huffmanTree
    where 
      (c, restoCodigo) = takeSymbol codigo huffmanTree
