...
toList (TBS tam tree) = toListRec 0 tree
  where 
    toListRec offSet (Leaf bitSet) = map (offSet +) (IntBits.toList bitSet)
    toListRec offSet (Node lt rt) = toListRec offSet lt ++ toListRec (offSet + bitsPerLeaf) rt
...
union tbs1@(TBS tam1 tree1) tbs2@(TBS tam2 tree2)
  | tam1 /= tam2 = error "TreeBitSet do not have the same dimension"
...
extendedUnion tbs1@(TBS tam1 tree1) tbs2@(TBS tam2 tree2)
  | tam1 < tam2   = TBS tam2 (extendedUnionRec tree1 tam1 tree2 tam2)
  | tam1 == tam2  = union tbs1 tbs2 
  | otherwise     = TBS tam1 (extendedUnionRec tree1 tam1 tree2 tam2)
  where 
    extendedUnionRec (Leaf bitSet1) _ (Leaf bitSet2) _ = Leaf (foldr IntBits.setBit' bitSet1 (IntBits.toList bitSet2))
    extendedUnionRec tree1@(Node lt1 rt1) tam1 tree2@(Node lt2 rt2) tam2
      | tam1 < tam2   = Node (extendedUnionRec tree1 tam1 lt2 (tam2 `div` 2)) rt2 
      | tam1 == tam2  = Node (extendedUnionRec lt1 (tam1 `div` 2) lt2 (tam2 `div` 2)) (extendedUnionRec rt1 (tam1 `div` 2) rt2 (tam2 `div` 2))
      | otherwise     = Node (extendedUnionRec lt1 (tam1 `div` 2) tree2 tam2) rt1 
