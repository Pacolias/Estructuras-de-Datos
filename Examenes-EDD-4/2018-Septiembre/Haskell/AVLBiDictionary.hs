...
isPermutation (Bi dk _) = null((D.values dk) \\ (D.keys dk))
...
orbitOf x (Bi dk _) = orbitOfRec x [x]
  where 
    orbitOfRec k orbit =
      case D.valueOf k dk of 
        Nothing -> error "?" 
        Just v  -> if v == x then orbit else orbitOfRec v (orbit ++ [v])

-- | Exercise k. cyclesOf

cyclesOf :: Ord a => BiDictionary a a -> [[a]]
cyclesOf biDict@(Bi dk _) = cyclesOfRec (D.keys dk)
  where 
    cyclesOfRec [] = []
    cyclesOfRec (k:ks) = orbita : (cyclesOfRec (ks \\ orbita))
      where 
        orbita = orbitOf k biDict
