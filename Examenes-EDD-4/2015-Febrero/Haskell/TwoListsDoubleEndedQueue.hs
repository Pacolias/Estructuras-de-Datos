...
-- Complexity: O(1)
first :: DEQue a -> a
first (DEQ [] [])    = error "first on empty DEQueue"
first (DEQ [] ys)    = first (DEQ (reverse (drop ((length ys) `div` 2) ys)) (take ((length ys) `div` 2) ys))
first (DEQ (x:xs) _) = x

-- Complexity: O(1)
last :: DEQue a -> a
last (DEQ [] [])    = error "last on empty DEQueue"
last (DEQ xs [])    = last (DEQ (reverse (drop ((length xs) `div` 2) xs)) (take ((length xs) `div` 2) xs))
last (DEQ _ (y:ys)) = y

-- Complexity: O(1)
deleteFirst :: DEQue a -> DEQue a
deleteFirst (DEQ [] [])     = error "deleteFirst on empty DEQueue"
...
deleteLast (DEQ [] [])     = error "deleteLast on empty DEQueue"
deleteLast (DEQ xs [])     = deleteLast (DEQ (reverse (drop ((length xs) `div` 2) xs)) (take ((length xs) `div` 2) xs))
