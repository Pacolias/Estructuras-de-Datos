...
public class Kruskal {
	public static <V,W> Set<WeightedEdge<V,W>> kruskal(WeightedGraph<V,W> g) {
		...
		while(!pq.isEmpty()){
			...
			repOrg = representante(firstEdge.source(), dict);
			repDst = representante(firstEdge.destination(), dict);

			if(!repOrg.equals(repDst)){
				dict.insert(repDst, firstEdge.source());
				t.insert(firstEdge);
			}
		}
