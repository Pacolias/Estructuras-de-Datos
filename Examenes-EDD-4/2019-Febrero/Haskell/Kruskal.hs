...
kruskal wg = kruskalRec (foldr (\v -> D.insert v v) D.empty (vertices wg)) (foldr Q.enqueue Q.empty (edges wg)) []
    where 
        kruskalRec dict pq t 
            | Q.isEmpty pq = t
            | otherwise    = 
                if repOrg /= repDst 
                    then kruskalRec (D.updateOrInsert repDst (const org) org dict) (Q.dequeue pq) (firstEdge : t) 
                else 
                    kruskalRec dict (Q.dequeue pq) t

                where 
                    firstEdge@(WE org w dst) = Q.first pq 
                    ...
                    representante v = 
                        case D.valueOf v dict of 
                            Nothing -> error "error"
                            Just u  -> if u == v then v else representante u
