...
addEdge (WG dict) u v p 
  | not((u `elem` D.keys dict) && (v `elem` D.keys dict)) = error "graph not defined on the given vertices"
  | otherwise                                             =
    case D.valueOf u dict of 
      Nothing    -> error "graph not defined on the given vertices"
      Just dictU -> WG (D.updateOrInsert u (const dictU') dictU' dict)
        where 
          dictU' = D.insert v p dictU

edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges (WG dict) = foldr (\(u, dictU) acc -> map (\(v, p) -> WE u p v) (D.keysValues dictU) ++ acc) [] (D.keysValues dict)
