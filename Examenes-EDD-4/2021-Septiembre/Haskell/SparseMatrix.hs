...
index (SM filMax colMax dict) fil col
...
add sm1@(SM fil1 col1 dict1) sm2@(SM fil2 col2 dict2)
    | fil1 /= fil2 || col1 /= col2 = error "the given matrices have not the same dimension"
    | otherwise                    = foldr (\(idx, num) acc -> update acc idx (value sm1 idx + num)) sm1 (D.keysValues dict2) 

-- Traspuesta de una matriz dispersa
transpose :: SparseMatrix -> SparseMatrix
transpose (SM fil col dict) = SM col fil (foldr (\(Idx f c, num) -> D.insert (Idx c f) num) D.empty (D.keysValues dict))

-- Crea una matriz dispersa a partir de una lista [fila, col, val, fila, col, val, ...]
fromList :: Int -> Int -> [Int] -> SparseMatrix
fromList fil col xs
    | length xs `mod` 3 /= 0 = error "fromList on list whose length is not multiple of 3"
    | otherwise              = SM fil col (foldr (uncurry D.insert) D.empty (fromListRec xs))
        where 
            fromListRec []          = []
            fromListRec (x:y:z:zs)  = (Idx x y, z) : fromListRec zs
