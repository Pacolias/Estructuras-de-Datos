...
size :: Trie a -> Int
size Empty         = 0 
size (Node x dict) = sizeValue x + sum [size child | child <- D.values dict]
...
strings Empty         = [""]
strings (Node x dict) 
  | sizeValue x == 1  = "" : suffix
  | otherwise         = suffix
  where 
    suffix = concat [map (c :) (strings childC) | (c, childC) <- D.keysValues dict]
...
fromList xs = fromListRec xs 0
  where 
    fromListRec [] _       = Empty 
    fromListRec (x:xs) acc = insert x acc (fromListRec xs (acc + 1))
