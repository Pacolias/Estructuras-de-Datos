...
localEquilibrium g src dst = foldr (&&) True [inDegree g v == outDegree g v | v <- vertices g, v /= src, v /= dst]
...
unifySourceAndSink g newSrc newDst 
    | hayNuevaFuente && hayNuevoSumidero    = unifySource (unifySink g)
    | hayNuevaFuente                        = unifySource g
    | hayNuevoSumidero                      = unifySink g
    | otherwise                             = g
    where 
        (fuentes,sumideros) = sourcesAndSinks g
        hayNuevaFuente = not(null fuentes)
        hayNuevoSumidero = not(null sumideros)

        unifySource g' = mkWeightedDiGraphEdges (newSrc : vertices g') (weightedDiEdges g' ++ (map (\f -> E newSrc (sum [w | (E x w y) <- weightedDiEdges g', x == f]) f) fuentes))
        unifySink g' = mkWeightedDiGraphEdges (newDst : vertices g') (weightedDiEdges g' ++ (map (\s -> E s (sum [w | (E x w y) <- weightedDiEdges g', y == s]) newDst) sumideros))