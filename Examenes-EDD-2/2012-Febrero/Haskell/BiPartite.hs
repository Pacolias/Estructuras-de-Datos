...
aux g dict stack
   ...
   | otherwise                 =
    case D.valueOf v dict of 
      Nothing  -> aux g (D.insert v c dict) (pushAll (S.pop stack) (map (\x -> (x, nextColor c)) (filter (\x -> not(D.isDefinedAt x dict)) (successors g v))))
      Just cd  -> if c /= cd then Nothing else aux g dict (S.pop stack)
      where 
        (v, c) = S.top stack
