...
remove g edge@(src, dst) = mkGraphEdges [v | v <- vertices g', degree g' v > 0] (edges g')
    where 
        g' = deleteEdge g edge 
...
extractCycle g v0 = extractCycleRec g v0 [v0]
    where 
        extractCycleRec g v vs
            | v == v0 && length vs > 1 = (g, reverse vs) 
            | otherwise                = extractCycleRec g' u (u:vs)
                where 
...
eulerianCycle g 
    ...
    | otherwise         = eulerianCycleRec g (head (vertices g)) []
        where 
            eulerianCycleRec g v ciclo
                | isEmpty g = ciclo 
                | otherwise = eulerianCycleRec g' (vertexInCommon g' cicloExtraido) (connectCycles ciclo cicloExtraido)
                    where 
                        (g', cicloExtraido) = extractCycle g v