...
vector n x
  | n < 0     = error "vector on negative integer n"
...
simplify lt@(Unif x) rt@(Unif y)  = if x /= y then Node lt rt else Unif x 
...
get :: Int -> Vector a -> a
get i (V tam tree)
  | i < 0 || i >= tam = error "index out of bounds"
  | otherwise         = getRec i tam tree 
    where 
      getRec :: Int -> Int -> Tree a -> a 
      getRec _ _ (Unif x) = x
      getRec i tam (Node lt rt)
        | i <= div tam 2 - 1 = getRec i (div tam 2) lt 
        | otherwise          = getRec (i - div tam 2) (div tam 2) rt
...
set i x (V tam tree)
  | i < 0 || i >= tam = error "index out of bounds"
  | otherwise         = V tam (setRec i tam x tree) 
    where 
      setRec :: (Eq a) => Int -> Int -> a -> Tree a -> Tree a
      setRec i tam x leaf@(Unif y)
        | tam == 1            = Unif x
        | i <= div tam 2      = simplify (setRec i (div tam 2) x leaf) leaf
        | otherwise           = simplify leaf (setRec (i - div tam 2) (div tam 2) x leaf)
      setRec i tam x (Node lt rt)
        | i <= div tam 2 - 1 = simplify (setRec i (div tam 2) x lt) rt
        | otherwise          = simplify lt (setRec (i - div tam 2) (div tam 2) x rt)
...
mapVector f (V tam tree) = V tam (mapVectorRec f tree)
  where 
    mapVectorRec :: (Eq a, Eq b) => (a -> b) -> Tree a -> Tree b 
    mapVectorRec f (Unif x)     = Unif (f x)
    mapVectorRec f (Node lt rt) = simplify (mapVectorRec f lt) (mapVectorRec f rt)

-------------------------------------------------------------------------------
-- | Exercise h.

filterVector :: (a -> Bool) -> Vector a -> [a]
filterVector p (V tam tree) = filterVectorRec p tam tree
  where 
    filterVectorRec :: (a -> Bool) -> Int -> Tree a -> [a]
    filterVectorRec p 1 (Unif x)        = [x | p x]
    filterVectorRec p tam leaf@(Unif x) = if p x then x : filterVectorRec p (tam - 1) leaf else []
    filterVectorRec p tam (Node lt rt)  = filterVectorRec p (div tam 2) lt ++ filterVectorRec p (div tam 2) rt

-------------------------------------------------------------------------------
-- | Exercise i.

depthOf :: Int -> Vector a -> Int 
depthOf i (V tam tree)
  | i < 0 || i >= tam = error "index out of bounds"
  | otherwise         = depthOfRec i tam 0 tree 
    where 
      depthOfRec :: Int -> Int -> Int -> Tree a -> Int 
      depthOfRec i tam depth (Unif x) = depth 
      depthOfRec i tam depth (Node lt rt)
        | i <= div tam 2 - 1 = depthOfRec i (div tam 2) (depth + 1) lt
        | otherwise          = depthOfRec (i - div tam 2) (div tam 2) (depth + 1) rt
