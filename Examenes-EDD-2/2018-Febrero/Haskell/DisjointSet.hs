...
root x ds@(DS dict) =
  case D.valueOf x dict of 
    Nothing -> Nothing 
    Just r  -> if x == r then Just r else root r ds 
...
union x y ds@(DS dict)
  | rootX < rootY   = DS (D.updateOrInsert rootY (const rootX) rootX dict)
  | rootX == rootY  = ds
  | otherwise       = DS (D.updateOrInsert rootX (const rootY) rootY dict)
  where 
    rootX = 
      case root x ds of
        Nothing -> x 
...
flatten ds@(DS dict) = DS (flattenRec (D.keys dict)) 
  where 
    ...
    flattenRec (x:xs) = 
      case root x ds of 
        Nothing -> flattenRec xs
        Just r  -> D.insert x r (flattenRec xs)

kinds :: Ord a => DisjointSet a -> [[a]]
kinds ds@(DS dict) = kindsRec (D.keys dict)
  where 
    kindsRec []     = []
    kindsRec (x:xs) = claseEq : kindsRec (xs \\ claseEq)
      where 
        claseEq = kind x ds
