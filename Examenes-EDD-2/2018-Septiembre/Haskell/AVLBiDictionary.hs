...
insert k v (Bi dk dv) = 
  case D.valueOf k dk of 
    ...
    Just valor  -> Bi (D.insert k v (D.delete k dk)) (D.insert v k (D.delete valor (D.delete v dv)))
...
toBiDictionary dict
  | length (D.keys dict) /= length (nub (D.values dict))  = error "toBiDictionary on not injective dictionary"
  | otherwise                                             = Bi dict (D.foldKeysValues (flip D.insert) D.empty dict)

-- | Exercise h. compose

compose :: (Ord a, Ord b, Ord c) => BiDictionary a b -> BiDictionary b c -> BiDictionary a c
compose (Bi dk1 _) (Bi dk2 _) = composeRec (D.keys dk1)
  where 
    composeRec []     = empty
    composeRec (a:as) =
      case D.valueOf a dk1 of 
        ...
        Just b  -> 
          case D.valueOf b dk2 of 
            Nothing -> composeRec as
            Just c  -> insert a c (composeRec as)


-- | Exercise i. isPermutation

isPermutation :: Ord a => BiDictionary a a -> Bool
isPermutation (Bi dk dv) = length (D.keys dk) == length (D.keys dv) && null (D.keys dk \\ D.values dk) && null (D.values dk \\ D.keys dk) 
...
orbitOf k biDict@(Bi dk dv)
  | not(isPermutation biDict) = error "orbitOf on not permutation dictionary"
  | otherwise                 = 
    case D.valueOf k dk of 
      Nothing -> []
      Just v  -> orbitOfRec v [k]
  where 
      orbitOfRec clave orbita =
        case D.valueOf clave dk of 
          Nothing -> orbita
          Just valor -> if clave /= k then clave : orbitOfRec valor orbita else orbita 

-- | Exercise k. cyclesOf

cyclesOf :: Ord a => BiDictionary a a -> [[a]]
cyclesOf biDict@(Bi dk dv) = cyclesOfRec (D.keys dk)
  where 
    cyclesOfRec []      = []
