...
weights palabra = foldr (\x -> D.insert x (length [c | c <- palabra, c == x])) D.empty palabra
...
huffmanLeaves palabra = foldr (\(x, n) -> PQ.enqueue (WLeaf x n)) PQ.empty (D.keysValues (weights palabra))
...
huffmanTree palabra
  | PQ.isEmpty (PQ.dequeue (huffmanLeaves palabra)) = error "huffmanTree: the string must have at least two different symbols"
  | otherwise                                       = huffmanTreeRec (huffmanLeaves palabra)
  where 
    huffmanTreeRec :: PQ.PQueue (WLeafTree Char) -> WLeafTree Char
    huffmanTreeRec hojas 
      | PQ.isEmpty (PQ.dequeue hojas) = PQ.first hojas
      | otherwise = huffmanTreeRec (PQ.enqueue mezcla colaParametro)
        where 
          primero = PQ.first hojas 
          segundo = PQ.first (PQ.dequeue hojas)
          mezcla = merge primero segundo 
          colaParametro = PQ.dequeue (PQ.dequeue hojas)
...
joinDics dict1 dict2 = foldr (uncurry D.insert) D.empty (D.keysValues dict1 ++ D.keysValues dict2)
...
prefixWith x dict = foldr (\(a, lista) -> D.insert a (x : lista)) D.empty (D.keysValues dict)
...
encode palabra dict = encodeRec palabra 
  where 
    encodeRec []      = []
    encodeRec (x:xs)  = 
      case D.valueOf x dict of 
        Nothing -> encodeRec xs 
        Just codigo -> codigo ++ encodeRec xs
...
takeSymbol codigo (WLeaf a _) = (a, codigo)
...
decode [] _ = []
decode codigo arbol = letra : decode code arbol
  where 
    (letra, code) = takeSymbol codigo arbol 
