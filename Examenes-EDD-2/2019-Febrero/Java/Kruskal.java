...
public class Kruskal {
	public static <V,W> Set<WeightedEdge<V,W>> kruskal(WeightedGraph<V,W> g) {
		...
		while(!pq.isEmpty()){
			...
			representanteSrc = representante(edge.source(), dict);
			representanteDst = representante(edge.destination(), dict);

			if(!representanteSrc.equals(representanteDst)){
				dict.insert(representanteDst, edge.source());
				t.insert(edge);
			}
		}
