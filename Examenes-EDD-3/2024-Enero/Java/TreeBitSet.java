...
//-------------------------------------------------------------------
  ...
  public TreeBitSet(long capacity) {
    if(!isValidCapacity(capacity) && capacity < 0)
  ...
  private TreeBitSet(long capacity, Tree root) {
	    this.capacity = capacity;
      this.root = root.cloneTree();
  }
  
  // * Exercise 10 * -
  public static TreeBitSet union(TreeBitSet set1, TreeBitSet set2) {
    if(set1.capacity() != set2.capacity())
      throw new IllegalArgumentException("sets have different capacities");

	  return new TreeBitSet(set1.capacity(), unionTrees(set1.root, set2.root));
  }
  
  private static Tree unionTrees(Tree tree1, Tree tree2) {
	    if(tree1 instanceof Leaf){
        ...
        return new Node(unionTrees(node1.left, node2.left), unionTrees(node1.right, node2.right));
      }
  }
  ...
  public static TreeBitSet extendedUnion(TreeBitSet set1, TreeBitSet set2) {
    if(set1.capacity() <= set2.capacity())
	    return new TreeBitSet(set2.capacity(), extendedUnionTrees(set1.root, set1.capacity(), set2.root, set2.capacity()));
    else 
	    return new TreeBitSet(set1.capacity(), extendedUnionTrees(set1.root, set1.capacity(), set2.root, set2.capacity()));
  }

  private static Tree extendedUnionTrees(Tree tree1, long capacity1, Tree tree2, long capacity2){
    	if(tree1 instanceof Leaf){
        Leaf leaf1 = (Leaf) tree1;
        Leaf leaf2 = (Leaf) tree2;
        ...
        if(capacity1 < capacity2)
          return new Node(extendedUnionTrees(tree1, capacity1, node2.left, capacity2/2), node2.right);
        else if(capacity2 < capacity1)
          return new Node(extendedUnionTrees(node1.left, capacity1/2, tree2, capacity2), node1.right);
        else
          return new Node(unionTrees(node1.left, node2.left), unionTrees(node1.right, node2.right));
      }
  }
