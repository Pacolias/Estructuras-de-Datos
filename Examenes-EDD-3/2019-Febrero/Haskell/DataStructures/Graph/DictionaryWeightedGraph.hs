...
addEdge g@(WG dict) v u w 
  | not(D.isDefinedAt v dict && D.isDefinedAt u dict) = error "some of the given vertices are not part of the graph"
  | otherwise                                         =
    case D.valueOf v dict of 
      Nothing         -> g 
      Just valueDict  -> WG (D.updateOrInsert v (const nuevoDict) nuevoDict dict)
        where 
          nuevoDict = D.insert u w valueDict

edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges (WG dict) = foldr (\(v,u,w) acc -> WE v w u : acc) [] (foldr (\(x,dictX) acc -> map (\(y,p) -> (x,y,p)) (D.keysValues dictX) ++ acc) [] (D.keysValues dict))

successors :: (Ord a, Show a) => WeightedGraph a w -> a -> [(a,w)]
successors (WG dict) v 
  | not(D.isDefinedAt v dict) = error "graph not defined on the given vertex"
