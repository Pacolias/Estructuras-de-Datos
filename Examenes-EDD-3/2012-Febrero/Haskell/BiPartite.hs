...
aux g dict stack
   ...
   | otherwise                 =
    case D.valueOf v dict of 
      Nothing -> aux g (D.insert v c dict) (pushAll stackDepilada (map (, nextColor c) (successors g v \\ D.keys dict)))
      Just cd -> if c /= cd then Nothing else aux g dict stackDepilada
    where 
      (v,c) = S.top stack
      stackDepilada = S.pop stack
