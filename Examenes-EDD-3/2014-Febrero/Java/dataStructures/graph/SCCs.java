...
public class SCCs {
    ...
    public static <V> DiGraph<V> reverseDiGraph(DiGraph<V> g) {
        Set<DiEdge<V>> edges = new HashSet<>();

        for(DiEdge<V> edge : g.edges())
            edges.add(new DiEdge<>(edge.target(), edge.source()));

        return DiGraph.fromEdges(g.vertices(), edges);
    }
