...
weights palabra = foldr (uncurry D.insert) D.empty (nub [(c, length (filter (c ==) palabra)) | c <- palabra])
...
huffmanLeaves palabra = foldr (\(c,p) -> PQ.enqueue (WLeaf c p)) PQ.empty (D.keysValues (weights palabra))
...
huffmanTree palabra 
  | D.size (weights palabra) < 2 = error "huffmanTree: the string must have at least two different symbols"
  | otherwise                    = huffmanTreeRec (huffmanLeaves palabra)
    where 
      huffmanTreeRec leavesPQ 
        | PQ.isEmpty (PQ.dequeue leavesPQ) = PQ.first leavesPQ 
        | otherwise                        = huffmanTreeRec colaParametro
          where 
            primero       = PQ.first leavesPQ 
            segundo       = PQ.first (PQ.dequeue leavesPQ)
            mezcla        = merge primero segundo 
            colaParametro = PQ.enqueue mezcla (PQ.dequeue (PQ.dequeue leavesPQ))
...
joinDics dict1 dict2 = foldr (uncurry D.insert) dict1 (D.keysValues dict2)
...
prefixWith x dict = foldr (\(y, xs) -> D.insert y (x:xs)) D.empty (D.keysValues dict)
...
encode palabra dict = encodeRec palabra 
  where 
    encodeRec []      = []
    encodeRec (c:cs)  = 
      case D.valueOf c dict of 
        Nothing   -> encodeRec cs 
        Just code -> code ++ encodeRec cs
...
decode code tree = decodeRec code 
  where 
    decodeRec []     = ""
    decodeRec codigo = letra : decodeRec resto
      where 
        (letra, resto) = takeSymbol codigo tree 
