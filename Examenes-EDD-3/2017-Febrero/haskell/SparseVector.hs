...
vector n x
  | n < 0     = error "vector on non positive integer"
...
get :: Int -> Vector a -> a
get i (V tam tree) = getRec i tam tree
  where 
    getRec _ _ (Unif x) = x
    getRec i tam (Node lt rt)
      | i < tam `div` 2 = getRec i (tam `div` 2) lt 
      | otherwise       = getRec (i - (tam `div` 2)) (tam `div` 2) rt

-------------------------------------------------------------------------------
-- | Exercise e.

set :: (Eq a) => Int -> a -> Vector a -> Vector a 
set i x (V tam tree) = V tam (setRec i tam tree)
  where 
    setRec i tam leaf@(Unif y)
      | tam == 1        = Unif x
      | i < tam `div` 2 = simplify (setRec i (tam `div` 2) leaf) leaf
      | otherwise       = simplify leaf (setRec (i - (tam `div` 2)) (tam `div` 2) leaf)
    setRec i tam (Node lt rt)
      | i < tam `div` 2 = simplify (setRec i (tam `div` 2) lt) rt 
      | otherwise       = simplify lt (setRec (i - (tam `div` 2)) (tam `div` 2) rt)
...
mapVector f (V tam tree) = V tam (mapVectorRec tree)
  where 
    mapVectorRec (Unif x)     = Unif (f x)
    mapVectorRec (Node lt rt) = simplify (mapVectorRec lt) (mapVectorRec rt)

-------------------------------------------------------------------------------
-- | Exercise h.

filterVector :: (Eq a) => (a -> Bool) -> Vector a -> [a] 
filterVector p (V tam tree) = filterVectorRec tam tree
  where 
    filterVectorRec tam (Unif x)      = [x | p x, i <- [0..(tam - 1)]]
    filterVectorRec tam (Node lt rt)  = filterVectorRec (tam `div` 2) lt ++ filterVectorRec (tam `div` 2) rt

-------------------------------------------------------------------------------
-- | Exercise i.

depthOf :: Int -> Vector a -> Int 
depthOf i (V tam tree) = depthOfRec i tam tree 0
  where 
    depthOfRec _ _ (Unif x) depth = depth 
    depthOfRec i tam (Node lt rt) depth
      | i < tam `div` 2 = depthOfRec i (tam `div` 2) lt (depth + 1)
      | otherwise       = depthOfRec (i - (tam `div` 2)) (tam `div` 2) rt (depth + 1)
