...
class LinearBinPacking {
    public static List<Bin> linearBinPacking(int initialCapacity, List<Integer> weights) {
        ...
        for(Integer w : weights)
            insert(res, w, initialCapacity);

        return res;
    }

    private static void insert(List<Bin> res, Integer w, int initialCapacity){
        int i = 0;
        boolean insertado = false;

        while(i < res.size() && !insertado){
            if(res.get(i).remainingCapacity() >= w){
                res.get(i).addObject(w);
                insertado = true;
            }
            i++;
        }
