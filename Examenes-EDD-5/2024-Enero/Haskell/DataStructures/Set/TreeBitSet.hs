...
toList (TBS tam tree) = toListRec tree tam 0
  where 
    toListRec (Leaf bitSet) _ offSet  = map (offSet +) (IntBits.toList bitSet) 
    toListRec (Node lt rt) tam offSet = toListRec lt half offSet ++ toListRec rt half (offSet + half)
      where 
        half = tam `div` 2 
...
union (TBS tam1 tree1) (TBS tam2 tree2)
  | tam1 /= tam2 = error "union on TBS with different dimension"
  | otherwise    = TBS tam1 (unionRec tree1 tree2)
    where
      unionRec (Leaf bitSet1) (Leaf bitSet2) = Leaf (foldr IntBits.setBit' bitSet1 (IntBits.toList bitSet2))
...
extendedUnion tbs1@(TBS tam1 tree1) tbs2@(TBS tam2 tree2)
  | tam1 == tam2 = tbs1 `union` tbs2
  | tam1 < tam2  = TBS tam2 (extendedUnionRec tam1 tree1 tam2 tree2)
  | otherwise    = TBS tam1 (extendedUnionRec tam1 tree1 tam2 tree2)
    where 
      extendedUnionRec _ (Leaf bitSet1) _ (Leaf bitSet2)    = Leaf (foldr IntBits.setBit' bitSet1 (IntBits.toList bitSet2))
      ...
      extendedUnionRec tam1 tree1@(Node lt1 rt1) tam2 tree2@(Node lt2 rt2)
        | tam1 < tam2  = Node (extendedUnionRec tam1 tree1 (tam2 `div` 2) lt2) rt2
        | tam1 == tam2 = Node (extendedUnionRec (tam1 `div` 2) lt1 (tam1 `div` 2) lt2) (extendedUnionRec (tam1 `div` 2) rt1 (tam1 `div` 2) rt2)
        | otherwise    = Node (extendedUnionRec (tam1 `div` 2) lt1 tam2 tree2) rt1
