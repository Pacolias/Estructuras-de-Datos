...
simplify leftLeaf@(Unif x) rightLeaf@(Unif y) = if x == y then rightLeaf else Node leftLeaf rightLeaf 
...
depthOf i (V tam tree) = depthOfRec i tam 0 tree 
  where 
    depthOfRec _ _ depth (Unif x) = depth
    depthOfRec i tam depth (Node lt rt)
      | i < half  = depthOfRec i half (depth+1) lt 
      | otherwise = depthOfRec (i - half) half (depth+1) rt 
      where 
        half = tam `div` 2
