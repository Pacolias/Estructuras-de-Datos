...
size :: Trie a -> Int
size Empty         = 0 
size (Node x dict) = sizeValue x + sum [size trie | trie <- D.values dict]

-- | = Exercise e - toTrie
toTrie :: Maybe (Trie a) -> Trie a
toTrie Nothing     = Empty 
toTrie (Just trie) = trie

-- | = Exercise f - childOf
childOf :: Char -> Trie a -> Trie a
childOf _ Empty         = Empty 
childOf c (Node _ dict) = toTrie (D.valueOf c dict)
...
update (Node x dict) c child = Node x (D.updateOrInsert c (const child) child dict)

-- | = Exercise i - insert
insert :: String -> a -> Trie a -> Trie a
insert [] v Empty         = Node (Just v) D.empty 
insert [] v (Node _ dict) = Node (Just v) dict
...
strings Empty                = [""]
strings (Node Nothing dict)  = concat [(map (c:) (strings t))| (c, t) <- D.keysValues dict] 
strings (Node (Just x) dict) = "" : concat [(map (c:) (strings t))| (c, t) <- D.keysValues dict] 

-- | = Exercise e2 - fromList
fromList :: [String] -> Trie Int
fromList xs = fromListRec xs 0 
  where 
    fromListRec [] _     = Empty 
    fromListRec (x:xs) i = insert x i (fromListRec xs (i + 1))
