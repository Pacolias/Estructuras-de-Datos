...
size :: Trie a -> Int
size Empty              = 0
size (Node final dict)  = sizeValue final + sum (map size (D.values dict))
...
childOf c t@(Node _ dict) = fromMaybe Empty (D.valueOf c dict)

-- | = Exercise g - search
search :: String -> Trie a -> Maybe a
search _ Empty                = Nothing 
search [] t@(Node final _)    = final 
search (c:cs) t@(Node _ dict) = search cs (childOf c t)

-- | = Exercise h - update
update :: Trie a -> Char -> Trie a -> Trie a
update Empty c child                = Node Nothing (D.insert c child D.empty)
update t@(Node final dict) c child  = Node final (D.updateOrInsert c (const child) child dict)  

-- | = Exercise i - insert
insert :: String -> a -> Trie a -> Trie a
insert [] v Empty             = Node (Just v) D.empty 
insert [] v (Node final dict) = Node (Just v) dict
insert (c:cs) v t             = update t c (insert cs v (childOf c t))
...
strings Empty = []
strings (Node final dict) =
  let suffixes = concatMap (\(c,t) -> map (c:) (strings t)) (D.keysValues dict)
  in if sizeValue final == 1 then "":suffixes else suffixes
...
fromList = addWords 0
  where
    addWords :: Int -> [String] -> Trie Int 
    addWords _ []     = Empty 
    addWords i (s:ss) = insert s i (addWords (i+1) ss) 
