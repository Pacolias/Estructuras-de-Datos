...
addEdge (WG dict) u v w
  | not(D.isDefinedAt u dict && D.isDefinedAt v dict) = error "addEdge on graph not defined on at least one of the given vertices"
  | otherwise                                         =
    case D.valueOf u dict of 
      Nothing    -> error "addEdge on graph not defined on at least one of the given vertices"
      Just dictU -> WG (D.updateOrInsert u (const dictU') dictU' dict)
        where 
          dictU' = D.updateOrInsert v (const w) w dictU

edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges (WG dict) = foldr (\(u, dictU) xs -> foldr (\(v, w) ys -> WE u w v : ys) [] (D.keysValues dictU) ++ xs) [] (D.keysValues dict)

successors :: (Ord a, Show a) => WeightedGraph a w -> a -> [(a,w)]
successors (WG dict) u = 
  case D.valueOf u dict of 
    Nothing    -> error "graph not defined on the given vertex"
    Just dictU -> D.keysValues dictU
