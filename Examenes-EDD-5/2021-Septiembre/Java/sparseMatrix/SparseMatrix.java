...
public class SparseMatrix {
    ...
    public SparseMatrix add(SparseMatrix other) {
        if(rows != other.rows || cols != other.cols)
            throw new IllegalArgumentException("Add on matrices with different dimension");
    ...
    public SparseMatrix transpose() {
        ...
        for(Index idx : values.keySet())
            sm.set(idx.getColumn(), idx.getRow(), values.get(idx));
    ...
    public static SparseMatrix fromList(int rows, int cols, List<Integer> data) {
        if(data.size() % 3 != 0)
            throw new IllegalArgumentException("The list length must be multiple of three");
