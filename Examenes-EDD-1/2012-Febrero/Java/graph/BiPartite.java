...
public class BiPartite<V> {
	...
	public BiPartite(Graph<V> graph) {
		...
		while (!stack.isEmpty()) {
			Pair<V, Color> par = stack.top();
			stack.pop();
			
			if(!isDefinedAt(par.first(), dict)){
				dict.insert(par.first(), par.second());

				Color siguienteColor = nextColor(par.second());

				for(V sucesor : graph.successors(par.first())){
					if(!isDefinedAt(sucesor, dict))
						stack.push(new Pair(sucesor, siguienteColor));
				}
			
			} else{
				Color cd = dict.valueOf(par.first());
