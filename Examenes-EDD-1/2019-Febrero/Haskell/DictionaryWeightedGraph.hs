...
addVertex (WG dict) k = 
  if D.isDefinedAt k dict
    then WG dict
    else WG (D.insert k D.empty dict) 

addEdge :: (Ord a, Show a) => WeightedGraph a w -> a -> a -> w -> WeightedGraph a w
addEdge (WG dict) v1 v2 p = 
  case D.valueOf v1 dict of 
    Nothing           -> error "addEdge on graph not defined at the given vertex" 
    Just diccionario -> WG (D.updateOrInsert v1 (const (D.insert v2 p diccionario)) (D.insert v2 p diccionario) dict)

edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges (WG dict) = nub [WE v w v' | (v, diccionario) <- D.keysValues dict, (v',w) <- D.keysValues diccionario]

successors :: (Ord a, Show a) => WeightedGraph a w -> a -> [(a,w)]
successors (WG dict) v = 
  case D.valueOf v dict of 
    Nothing           -> error "succesors on graph not defined at the given vertex"
    Just diccionario  -> D.keysValues diccionario
