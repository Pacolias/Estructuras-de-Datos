...
isElem elem (DS dict) = D.isDefinedAt elem dict
...
root x ds@(DS dict) = 
  case D.valueOf x dict of 
    Nothing -> Nothing 
    Just y  -> if x == y then Just y else root y ds
...
areConnected x y ds
  | not(isElem x ds) || not(isElem y ds) = False 
  | otherwise = root x ds == root y ds

-- | Exercise 8. kind

kind :: Ord a => a -> DisjointSet a -> [a]
kind x ds@(DS dict) = filter (\y -> areConnected x y ds) (D.keys dict)
 
-- | Exercise 9. union

union :: Ord a => a -> a -> DisjointSet a -> DisjointSet a
union x y ds@(DS dict) =
  case (root x ds, root y ds) of
    (Just raizX, Just raizY)
      | raizX > raizY -> DS (D.updateOrInsert raizX (const raizY) raizY dict)
      | raizX < raizY -> DS (D.updateOrInsert raizY (const raizX) raizX dict)
      | otherwise     -> ds
    _ -> ds 
...
flatten ds@(DS dict) = DS (diccionarioAplanado (D.keys dict))
  where 
    diccionarioAplanado [] = D.empty 
    diccionarioAplanado (x:xs) = 
      case root x ds of 
        Nothing     -> diccionarioAplanado xs 
        Just raizX  -> D.insert x raizX (diccionarioAplanado xs)


kinds :: Ord a => DisjointSet a -> [[a]]
kinds ds@(DS dict) = calcularClases (D.keys dict) []
  where 
    calcularClases [] _ = []
    calcularClases (x:xs) ys = 
      case root x ds of 
        Nothing     -> calcularClases xs ys 
        Just raizX  -> if raizX `elem` ys then calcularClases xs ys else kind x ds : calcularClases xs (raizX:ys)
...
>>> flatten (add 2 (add 1 empty))
DictionaryDisjointSet((1,1),(2,2))

>>> flatten (union 1 2 (add 2 (add 1 empty)))
DictionaryDisjointSet((1,1),(2,1))

>>> flatten (union 1 3 (add 3 (add 2 (add 1 empty))))
DictionaryDisjointSet((1,1),(2,2),(3,1))

>>> flatten (union 1 2 (union 2 3 (add 3 (add 2 (add 1 empty)))))
DictionaryDisjointSet((1,1),(2,1),(3,1))
...
>>> kinds (add 2 (add 1 empty))
[[1],[2]]

>>> kinds (union 1 2 (add 2 (add 1 empty)))
[[1,2]]

>>> kinds (union 1 3 (add 3 (add 2 (add 1 empty))))
[[1,3],[2]]

>>> kinds (union 1 2 (union 2 3 (add 3 (add 2 (add 1 empty)))))
[[1,2,3]]

>>> kinds (union 1 2 (union 3 4 (add 4 (add 3 (add 2 (add 1 empty))))))
[[1,2],[3,4]]
