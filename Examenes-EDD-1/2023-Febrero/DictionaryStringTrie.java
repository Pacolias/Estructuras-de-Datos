...
public class DictionaryStringTrie<V> {
  ...
  ****************************************************************************/
  ...
  public boolean isEmpty() {
    return root.isEmpty() && root.value == null;
  }
  ...
  protected static <V> int size(Node<V> n) {
    ...
    for(Character letra : n.children.keys()){
      resultado += size(n.children.valueOf(letra));
    }
  ...
  protected static <V> Node<V> childOf(char c, Node<V> node) {
    return node.children.isDefinedAt(c) ? node.children.valueOf(c) : null;
  }
  ...
  protected static <V> V search(String str, Node<V> node) {
    ...
    if(str.isEmpty())
      ...
      resultado = search(str.substring(1), childOf(str.charAt(0), node));
  ...
  protected static <V> Node<V> insert(String str, V value, Node<V> node) {
    ...
    if(str.isEmpty()){
      ...
      Node<V> nodoHijo = childOf(str.charAt(0), node);
      Node<V> hijoInsertado =  insert(str.substring(1), value, nodoHijo);
      node.children.insert(str.charAt(0), hijoInsertado);
    }
  ...
  protected static <V> List<String> strings(Node<V> node) {
    ...
    stringsAux(node, "", listaPalabras);
    return listaPalabras;
  }

  private static <V> void stringsAux(Node<V> node, String prefijo, List<String> listaPalabras){
    if(node == null) return;
    ...
    for(Character c : node.children.keys()){
      Node<V> hijo = childOf(c, node);
      stringsAux(hijo, prefijo + c, listaPalabras);
    }
  }

  // | = Exercise e2 - fromList
  public static DictionaryStringTrie<Integer> fromList(List<String> list) {
    DictionaryStringTrie<Integer> resultado = new DictionaryStringTrie<>();
    int value = 0;

    for(String palabra : list){
      resultado.root = insert(palabra, value);
      value++;
    }
