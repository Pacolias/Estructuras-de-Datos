  ...
  | otherwise                    = huffmanTreeRec (huffmanLeaves palabra)
    where 
      huffmanTreeRec pq 
        ...
        | otherwise                  = huffmanTreeRec pq'
          where 
            primero = PQ.first pq 
            segundo = PQ.first (PQ.dequeue pq)
            mezcla  = merge primero segundo 
            pq'     = PQ.enqueue mezcla (PQ.dequeue (PQ.dequeue pq))
...
encode (c:cs) dict = 
  case D.valueOf c dict of 
    Nothing   -> encode cs dict 
    Just code -> code ++ encode cs dict 
...
takeSymbol code (WLeaf c _)       = (c, code)
...
decode [] _      = ""
decode code tree = c : decode rest tree 
    where 
      (c, rest) = takeSymbol code tree
