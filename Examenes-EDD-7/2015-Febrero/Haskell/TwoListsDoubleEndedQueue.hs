...
-- Complexity: O(1)
first :: DEQue a -> a
first (DEQ [] [])   = error "error: first on empty DEQ"
first (DEQ (x:_) _) = x
...
-- Complexity: O(1)
last :: DEQue a -> a
last (DEQ [] [])    = error "error: last on empty DEQ"
last (DEQ _ (y:ys)) = y
...
deleteFirst (DEQ [] [])     = error "error: deleteFirst on empty DEQ"
...
deleteLast (DEQ [] [])     = error "error: deleteLast on empty DEQ"
