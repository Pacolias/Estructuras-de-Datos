...
addEdge (WG dict) u v w 
  ...
  | otherwise                                          = WG (D.updateOrInsert u (const dictU') dictU' dict)
    where 
      dictU' = 
        case D.valueOf u dict of 
          Nothing    -> error ""
          Just dictU -> D.updateOrInsert v (const w) w dictU
...
successors (WG dict) u = D.keysValues dictU 
  where 
    dictU = fromMaybe D.empty (D.valueOf u dict)
