...
isEulerian g
    | isEmpty g = False 
    | otherwise = and [even (degree g v) | v <- vertices g] 

-- H.2)
remove :: (Eq a) => Graph a -> (a,a) -> Graph a
remove g (u,v) = mkGraphEdges [w | w <- vertices g', degree g' w > 0] (edges g') 
    where 
        g' = deleteEdge g (u,v)
...
extractCycle g v0 = extractCycleRec g v0 [v0]
    where 
        extractCycleRec g v sol 
            | v == v0 && length sol > 1 = (g, reverse sol)
            | otherwise                 = extractCycleRec g' u sol'
                where 
...
vertexInCommon g vs = head [v | v <- vertices g, v `elem` vs]

-- H.6) 
eulerianCycle :: Eq a => Graph a -> Path a
eulerianCycle g 
    | not (isEulerian g) = error "error: eulerianCycle on not eulerian graph"
    | otherwise          = eulerianCycleRec g (head (vertices g)) []
        where 
            eulerianCycleRec g v xs
                | isEmpty g = xs 
                | otherwise = eulerianCycleRec g' u zs 
                    where 
                        (g', ys) = extractCycle g v 
                        zs       = connectCycles xs ys 
                        u        = vertexInCommon g' zs
