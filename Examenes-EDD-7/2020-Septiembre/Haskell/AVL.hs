...
addObject w (B capacity ws)
  | w > capacity = error "addObject on bin without enough capacity"
...
addNewBin bin (Node x _ _ lt rt)
...
addFirst capacity w avl@(Node x h _ lt rt)
  | w > maxRemainingCapacity avl = addNewBin (addObject w (emptyBin capacity)) avl 
...
linearBinPacking capacity = foldr (add) SEmpty
  where 
    add w SEmpty                   = SNode (addObject w (emptyBin capacity)) SEmpty
    add w (SNode bin seq)
      | remainingCapacity bin >= w = SNode (addObject w bin) seq 
      | otherwise                  = SNode bin (add w seq)
