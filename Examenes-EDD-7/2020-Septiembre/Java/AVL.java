...
class Bin {
    ...
    public void addObject(int weight) {
        if(remainingCapacity < weight)
            throw new IllegalArgumentException("addObject on bin without enough capacity");
...
class LinearBinPacking {
    public static List<Bin> linearBinPacking(int initialCapacity, List<Integer> weights) {
        ...
        for(Integer w : weights)
            add(initialCapacity, w, res);

        return res;
    }

    private static void add(int initialCapacity, Integer weight, List<Bin> res){
        boolean added = false;
        Iterator<Bin> bins = res.iterator();
        Bin current;

        while(!added && bins.hasNext()){
            current = bins.next();

            if(!added && current.remainingCapacity() >= weight){
                added = true;
