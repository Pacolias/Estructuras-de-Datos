...
vector n x 
  | n < 0     = error "error: vector on non positive integer"
...
simplify lLeaf@(Unif x) rLeaf@(Unif y) = if x == y then rLeaf else Node lLeaf rLeaf 
...
get i (V tam tree) = getRec i tam tree 
  where 
    ...
    getRec i tam (Node lt rt) 
      | i < half  = getRec i half lt 
      | otherwise = getRec (i - half) half rt 
        where 
          half = tam `div` 2
...
set i x (V tam tree) = V tam (setRec i tam tree) 
  where 
    setRec i tam leaf@(Unif _)
      | tam == 1  = Unif x 
      | i < half  = simplify (setRec i half leaf) leaf
      | otherwise = simplify leaf (setRec (i - half) half leaf)
        where 
          half = tam `div` 2 
      
    setRec i tam (Node lt rt) 
      | i < half  = simplify (setRec i half lt) rt 
      | otherwise = simplify lt (setRec (i - half) half rt)
        where 
          half = tam `div` 2
...
filterVector p (V tam tree) = filterVectorRec tam tree 
  where 
    filterVectorRec tam (Unif x)     = [x | i <- [1..tam], p x]
    filterVectorRec tam (Node lt rt) = filterVectorRec half lt ++ filterVectorRec half rt 
      where 
        half = tam `div` 2
...
depthOf i (V tam tree) = depthOfRec i tam tree 0 
  where 
    depthOfRec _ _ (Unif _) depth = depth 
    depthOfRec i tam (Node lt rt) depth
      | i < half  = depthOfRec i half lt (depth + 1)
      | otherwise = depthOfRec (i - half) half rt (depth + 1)
        where 
          half = tam `div` 2
