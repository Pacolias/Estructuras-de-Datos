...
weights palabra = foldr (\c -> D.insert c (length [c' | c' <- palabra, c' == c])) D.empty palabra
...
joinDics dict1 dict2 = foldr (\(k,v) -> D.insert k v) dict2 (D.keysValues dict1)
...
prefixWith x dict = foldr (\(c,xs) -> D.insert c (x:xs)) D.empty (D.keysValues dict)
...
huffmanCode (WLeaf c n)     = D.insert c [] D.empty 
...
decode [] _ = []
decode xs huffmanTree = c : decode resto huffmanTree
  where 
    (c, resto) = takeSymbol xs huffmanTree
