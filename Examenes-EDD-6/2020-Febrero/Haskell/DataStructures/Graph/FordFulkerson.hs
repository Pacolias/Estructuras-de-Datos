...
addFlow x y p sol
    ...
    | not(null [w | E u w v <- sol, u == y, v == x, w == p]) = [E u w v | E u w v <- sol, not(u == x && v == y && w == p)]
...
maxFlowMinCut g src dst set = sum [p | E x p y <- weightedDiEdges g, x `elem` k, y `notElem` k] - sum [p | E x p y <- weightedDiEdges g, x `notElem` k, y `elem` k]
    where 
...
sourcesAndSinks g = ([v | v <- vertices v, inDegree g v == 0], [v | v <- vertices v, outDegree g v == 0])
...
unifySourceAndSink g newSrc newDst
    ...
    | otherwise                              = g
        where 
            ...
            pesoSrc v = sum [w | E x w y <- weightedDiEdges g, x == v]
            pesoDst v = sum [w | E x w y <- weightedDiEdges g, y == v]