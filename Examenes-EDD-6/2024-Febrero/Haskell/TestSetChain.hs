...
testSize = TestCase $ do
  ...
  assertEqual "size after validate" 2 (size sc2) -- ojo: size de validated set
  let sc3 = add tx3 sc2
  assertEqual "size after add post validate" 2 (size sc3)
...
testValidateDuplicate = TestCase $ do
  ...
  result <- (evaluate (validate sc3) >> return True)
            `catch` (\(ErrorCall _) -> return False)
