...
getEpoch trans (SC _ history _) = getEpochRec (D.keys history)
  where 
    getEpochRec []     = -1
    getEpochRec (e:es) = 
      case D.valueOf e history of 
        Nothing       -> -1 
        Just transSet -> if S.isElem trans transSet then e else getEpochRec es
...
validate sc@(SC mempool history epoch)
  ...
  | not(S.isEmpty (mempool `S.intersection` transSet)) = error "transaction already validated"
  | otherwise                                          = SC S.empty (D.insert epoch mempool history) (epoch+1)
  where 
    transSet = foldr S.union S.empty (D.values history)
...
fold f z (SC _ history _) = S.fold f z transSet
  where 
    transSet = foldr S.union S.empty (D.values history)
