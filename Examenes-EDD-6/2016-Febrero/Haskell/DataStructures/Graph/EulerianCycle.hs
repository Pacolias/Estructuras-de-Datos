...
remove g (u,v) = mkGraphEdges [v | v <- vertices g', degree g' v /= 0] (edges g')
    where 
        g' = deleteEdge g (u,v)
...
extractCycle g v0 = extractCycleRec g v0 [v0]
    where 
        extractCycleRec g v sol 
            ...
            | otherwise                 = if u == v then extractCycleRec g' u sol else extractCycleRec g' u (u:sol)
                where
...
eulerianCycle g
    | not(isEulerian g) = error "eulerianCycle on not eulerian graph g"
