...
strings Empty           = [""]
strings t@(Node x dict) 
  | isNothing x = concat [map (c :) (strings trie)| (c, trie) <- D.keysValues dict]
  | otherwise   = "" : concat [map (c :) (strings trie)| (c, trie) <- D.keysValues dict]
...
fromList xs = fromListRec xs 0 
  where 
    fromListRec [] _       = Empty 
    fromListRec (p:ps) acc = insert p acc (fromListRec ps (acc + 1))
