...
size :: TreeBitSet -> Int
size (TBS tam tree) = sizeRec tam tree 
  where 
    sizeRec _ (Leaf bitSet) = IntBits.countBits bitSet 
    sizeRec tam (Node lt rt) = sizeRec half lt + sizeRec half rt 
      where 
        half = tam `div` 2
...
contains i (TBS tam tree) = containsRec i tam tree 
  where 
...
add i tbs@(TBS tam tree) 
  | outOfRange tbs i = error "index out of bounds"
...
union (TBS tam1 tree1) (TBS tam2 tree2)
  ...
  | otherwise    = TBS tam1 (unionRec tam1 tree1 tree2)
    where 
      unionRec _ (Leaf bitSet1) (Leaf bitSet2)   = Leaf (foldr IntBits.setBit' bitSet2 (IntBits.toList bitSet1))
      unionRec tam (Node lt1 rt1) (Node lt2 rt2) = Node (unionRec half lt1 lt2) (unionRec half rt1 rt2)
        where 
          half = tam `div` 2

-- | Función para realizar la unión extendida de dos TreeBitSets
extendedUnion :: TreeBitSet -> TreeBitSet -> TreeBitSet
extendedUnion (TBS tam1 tree1) (TBS tam2 tree2)
  | tam1 < tam2  = TBS tam2 (extendedUnionRec tam1 tree1 tam2 tree2)
  | tam1 == tam2 = TBS tam2 (extendedUnionRec tam1 tree1 tam2 tree2)
  | otherwise    = TBS tam1 (extendedUnionRec tam1 tree1 tam2 tree2)
  where 
    extendedUnionRec _ (Leaf bitSet1) _ (Leaf bitSet2) = Leaf (foldr IntBits.setBit' bitSet2 (IntBits.toList bitSet1))
    extendedUnionRec tam1 leaf@(Leaf _) tam2 (Node lt rt) = Node (extendedUnionRec tam1 leaf (tam2 `div` 2) lt) rt
    extendedUnionRec tam1 (Node lt rt) tam2 leaf@(Leaf _) = Node (extendedUnionRec (tam1 `div` 2) lt tam2 leaf) rt
    extendedUnionRec tam1 n1@(Node lt1 rt1) tam2 n2@(Node lt2 rt2)
      | tam1 < tam2  = Node (extendedUnionRec tam1 n1 (tam2 `div` 2) lt2) rt2
      | tam1 == tam2 = Node (extendedUnionRec (tam1 `div` 2) lt1 (tam2 `div` 2) lt2) (extendedUnionRec (tam1 `div` 2) rt1 (tam2 `div` 2) rt2)
      | otherwise    = Node (extendedUnionRec (tam1 `div` 2) lt1 tam2 n2) rt1 
